...
namespace _2023_GC_A2_Partiel_POO.Tests.Level_2
{
    public class FightMoreTests
    {
        ...
        [Test]
        public void CharacterReceiveCriticalAttack()
        {
            var pikachu = new Character(100, 50, 30, 20, TYPE.NORMAL);
            var criticalPunch = new CriticalPunch();
            var normalPunch = new Punch();

            pikachu.CriticalChance = 0.2; // 20% de chance critique

            // Test avec un coup critique
            pikachu.ReceiveAttack(criticalPunch);
            Assert.That(pikachu.CurrentHealth, Is.EqualTo(100 - (int)(criticalPunch.Power * 1.5 - pikachu.Defense)));

            // Test sans coup critique
            pikachu.ReceiveAttack(normalPunch);
            Assert.That(pikachu.CurrentHealth, Is.EqualTo(100 - (normalPunch.Power - pikachu.Defense)));
        }
    }
}
